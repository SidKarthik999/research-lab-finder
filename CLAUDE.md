...
Future goals (from docs/ROADMAP.md): professor detail pages with LLM-generated summaries and cold-email drafting, backed by multi-provider accounts (Google + email/password) and a student profile (Phase 5A — **next up**, promoted ahead of 3 and 4 on 2026-08-06), then broader institution coverage (Phase 3), automated lab ingestion (Phase 4), and an opportunities layer (Phase 5B). The backend is intentionally structured so an `/api/*` endpoint calling an LLM can be added without touching the search path. Phase numbers in ROADMAP.md are not renumbered when priorities change, because code comments here and in `src/ingestion/openalex.py` cite them.
...
- **Contact panel (Phase 2)**: each card's contact line combines stored data (email, `Professor.website` — backfilled by `researcher_urls.py`, ORCID) with two links computed client-side from data already in the `/api/search` response, no extra request or storage — **neither is a guaranteed direct hit; both are search links that still require a click to find the right result, and the labels ("Find on Google Scholar", "Search for `<name>` on `<institution>`'s site") are worded to not overclaim otherwise**:
  - Google Scholar author search (`scholarSearchUrl()`, `mauthors=<name>` only). Appending the institution was tried and reverted — Scholar's `mauthors` match appears to check the query against the profile's own listed affiliation text, so if that doesn't literally match what's passed (an abbreviation, an outdated affiliation, etc.) it filters the person out entirely rather than narrowing correctly; confirmed this regressed a real profile. Don't re-add institution here without a way to verify it doesn't happen again.
  - Institution site-search (`directorySearchUrl()`, `site:<institution domain> "<professor name>"` via Google — domain extracted from `institution_website`). The name **must** be quoted, or Google treats the two name tokens as independent keywords anywhere on the site instead of requiring them together, which badly hurts precision. There is no way to build a true direct link to each institution's actual faculty directory without institution-specific knowledge of that school's site structure — that's the "per-institution directory adapters" idea in Phase 4/ROADMAP.md, worth building only if this search-link approach turns out to actually be the bottleneck.
...
`AppUser` (the person) and `AuthIdentity` (one row per login method) are separate tables — multi-provider from the start, so Google is one identity a person can have rather than *being* the account. `StudentProfile` is 1:1 on `AppUser`; `EmailDraft` records what's been generated. All four, plus `Professor.ai_summary`/`ai_summary_generated_at`, come from migration `004_accounts_and_summaries.sql`. Insert/query helpers for all of this live in `src/database.py` alongside the OpenAlex ones, following the same upsert conventions — except `upsert_student_profile`, which uses `EXCLUDED` (full replace) rather than `COALESCE`, since that's the student directly editing their own form and a cleared field should actually clear.

**The account-linking rule:** a new sign-in only auto-links to an existing `AppUser` by email when the *incoming* assertion of that email is provider-verified. Auto-linking on an unverified email is a known account-takeover path — an attacker registers under someone else's address, that person later signs in with Google, and the two accounts merge into the attacker's. `insert_user()`'s `ON CONFLICT` clause enforces the safe half of this by construction: `email_verified = AppUser.email_verified OR EXCLUDED.email_verified` can only ever upgrade the flag, never downgrade it.

**Password signup doesn't touch the database until the email is proven controlled.** `POST /api/auth/signup` hashes the password and mints a signed, time-limited token (`backend/tokens.py`, itsdangerous) carrying `{email, password_hash}` — no `AppUser` or `AuthIdentity` row is created or modified yet. Only `POST /api/auth/verify-email`, on redeeming that token, calls `insert_user()` + `insert_auth_identity()`. This closes the takeover case from the other direction: if someone signs up with an email that already belongs to a verified Google-only account, nothing about that account changes unless whoever clicks the link actually controls the inbox. Verification and reset tokens use different itsdangerous salts so one can't be replayed as the other.

**`insert_auth_identity()`'s `ON CONFLICT` branch deliberately writes nothing but `updated_at`** — never `user_id`, never `password_hash`. Google sign-in calls this on every login, so it has to be idempotent, but silently reassigning an existing identity's `user_id` or clobbering its password hash on a routine repeat call would itself be the takeover bug. Password changes go through the dedicated `update_identity_password()` instead — `POST /api/auth/reset` calls it when a password identity already exists, and falls through to a fresh `insert_auth_identity()` (which *does* accept a hash on the initial `INSERT`, just not the conflict branch) when this is the first password an account has ever had.

**No enumeration surface.** `POST /api/auth/login` returns the identical error whether the email doesn't exist or the password is wrong. `POST /api/auth/forgot` always returns the identical "check your email" message whether or not the account exists. Both are covered by tests exercising the actual route, not just the underlying functions.

**`backend/email.py`'s `send_email()`** is a seam: the dev backend (`EMAIL_BACKEND` unset or `"console"`) prints to stdout, so signup/verification/reset work end-to-end with zero external dependency. Production picks a transactional email provider and sets `EMAIL_BACKEND` — this is the one decision Phase 5A's plan deliberately left open; see `docs/ROADMAP.md` Phase 6.2.

**Sessions** are a signed, `HttpOnly` cookie via Starlette's `SessionMiddleware` (added in `backend/main.py`, keyed by `SESSION_SECRET`) — no server-side session store. `https_only` is gated on `ENV=production` so local HTTP dev still works; `SESSION_SECRET` has no default and fails loudly at startup if unset, rather than silently signing cookies with a guessable key. `backend/sessions.py`'s `current_user` is the FastAPI dependency every authenticated route takes; a cookie referencing a since-deleted user clears itself rather than leaving a dangling session.

**Split for testability**, same principle as `build_search_query()` and the ingestion pipeline's pure/network split: `backend/tokens.py` (signing/expiry), `backend/security.py` (argon2 hash/verify), and `backend/google_auth.py`'s `extract_google_identity()` (claims → identity, including the verified-email gate) are all pure — no DB, no network — and covered in `tests/`. `verify_google_id_token()` itself (the actual network+crypto call to Google) isn't unit tested, mirroring how OpenAlex's fetch functions aren't either.
