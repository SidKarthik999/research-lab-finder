...
- Schema lives in `database/schema.sql`, ad-hoc queries in `database/queries.sql`. There is no migration tool — schema changes are applied by hand (e.g. `psql -d research_lab_finder -f database/schema.sql`). `schema.sql` currently starts with `DROP TABLE IF EXISTS ... CASCADE` for the shelved tables (Lab/Publication/etc.) as a one-time cleanup step from the 2026-08-01 rebuild; it's safe to leave in since those `DROP`s are no-ops once the tables are already gone.
